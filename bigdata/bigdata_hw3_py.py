...
Original file is located at
    https://colab.research.google.com/drive/10ulFBntYRMk7Zb2LABVnmVb4vwaCIbUg
"""
...
df["AD_CLICK"]=df["AD_CLICK"].fillna(0)
...
flag2=0
for i in range(0,df.shape[0]):
  if(df["AD_EXPO"][i]==0 and df["AD_TIME"][i]!=0):
    print("錯誤列為第",i,"列")
    flag2=1
if(flag2==0):
  print("狀況二是否有錯誤:否")
...
flag3=0
for i in range(0,df.shape[0]):
  if(df["AD_CLICK"][i]==0 and df["AD_TIME"][i]!=0):
    print("錯誤列為第",i,"列")
    flag3=1
if(flag3==0):
  print("狀況三是否有錯誤:否")
...
data = df.groupby(["SEX","AGE","MARITAL_STATUS","CITY","DISTRICT","EDUCATION","PRIVATE_VEHICLE","COMMUTE","CHILDREN","ANNU_INCOME","DATE","PAYBY","QUANTITY","MCC_CAT","AMOUNT","AD_EXPO","AD_CLICK"])
c=0
datatable=[]
datatable.append(["男","女","30~39歲","40~49歲","50~59歲","其他年齡","北部","中部","其他地區","年收入30w",
        "年收入50w","年收入其他","服飾用品","美妝保健","電器用品","生活百貨","教育娛樂","食品飲料","其他",
        "貨到付款","信用卡","ATM轉帳","已婚","未婚","有交通工具","無交通工具","有小孩","沒有小孩","通勤","無通勤","有無點擊廣告"])
...
#居住地============================================
  if(key[3]=="台北市" or key[3]=="新北市" or key[3]=="基隆市" or key[3]=="新竹市"or key[3]=="桃園市"or key[3]=="新竹縣"):
    location_n=1
    location_c=0
    location_other=0
  elif(key[3]=="台中市" or key[3]=="苗栗縣" or key[3]=="彰化縣" or key[3]=="南投縣"or key[3]=="雲林縣"):
    location_n=0
    location_c=1
    location_other=0
  else:
...
#年收入============================================
  ...
  if(int(temp)>300000 and int(temp)<=500000):
    income_30=1
    income_50=0
    income_other=0
...
#==============================================
  ...
  #男、女、
  #年齡(30~39)、年齡(40~49)、年齡(50~59)、年齡(other)
  #北部、中部、其他地區、
  #30w~50w、50~70w、其他
  #消費種類(7種)
  #payby 信用卡 1 2 3
  #MARITAL_STATUS yes no
  #PRIVATE_VEHICLE yes no
  #children yes no
  #COMMUTE yes no
  #投放15
  #點擊16
...
#投放與否============================================
  if(key[15]==1 and key[16]==1):
    
    CLICK=1
    datatable.append([male,female,age_30,age_40,age_50,age_other,location_n,location_c,location_other,income_30,income_50,income_other
            ,consume_1,consume_2,consume_3,consume_4,consume_5,consume_6,consume_7
            ,pay_1,pay_2,pay_3,MARITAL_STATUS_y,MARITAL_STATUS_n,PRIVATE_VEHICLE_y,PRIVATE_VEHICLE_n
            ,children_y,children_n,COMMUTE_y,COMMUTE_n,CLICK])#性別、年齡、城市、居住、日期、消費種類、付款別、訂購量、交易金額
    print(male,female,age_30,age_40,age_50,age_other,location_n,location_c,location_other,income_30,income_50,income_other
            ,consume_1,consume_2,consume_3,consume_4,consume_5,consume_6,consume_7
            ,pay_1,pay_2,pay_3,MARITAL_STATUS_y,MARITAL_STATUS_n,PRIVATE_VEHICLE_y,PRIVATE_VEHICLE_n
            ,children_y,children_n,COMMUTE_y,COMMUTE_n,CLICK)
with open('/content/drive/My Drive/Colab Notebooks/bigdata/hw3_dataframe1.csv', 'w', newline='') as csvfile:
...
import pandas as pd
    ...
    #         ,consume_1,consume_2,consume_3,consume_4,consume_5,consume_6,consume_7
    #         ,pay_1,pay_2,pay_3,MARITAL_STATUS_y,MARITAL_STATUS_n,PRIVATE_VEHICLE_y,PRIVATE_VEHICLE_n
    #         ,children_y,children_n,COMMUTE_y,COMMUTE_n,CLICK_y,CLICK_n])#性別、年齡、城市、居住、日期、消費種類、付款別、訂購量、交易金額
column=["男","女","30~39歲","40~49歲","50~59歲","其他年齡","北部","中部","其他地區","年收入30w",
        "年收入50w","年收入其他","服飾用品","美妝保健","電器用品","生活百貨","教育娛樂","食品飲料","其他",
        "貨到付款","信用卡","ATM轉帳","已婚","未婚","有交通工具","無交通工具","有小孩","沒有小孩","通勤","無通勤","有無點擊廣告"]
df2 = pd.read_csv('/content/drive/My Drive/Colab Notebooks/bigdata/hw3_dataframe.csv')
df2=pd.DataFrame(df2)
# df2=df2.rename(columns={"0":"男","1":"女","0.1":"30~39歲","0.2":"40~49歲","0.3":"50~59歲","1.1":"其他年齡","1.2":"北部","0.4":"中部","0.5":"其他地區"
# ,"0.6":"年收入30w","1.3":"年收入50w","0.7":"年收入其他"
# ,"1.4":"服飾用品","0.8":"美妝保健","0.9":"電器用品","0.10":"生活百貨","0.11":"教育娛樂","0.12":"食品飲料","0.13":"其他","1.5":"貨到付款",
# "0.14":"信用卡","0.15":"ATM轉帳","0.16":"已婚","1.6":"未婚","1.7":"有交通工具","0.17":"無交通工具"
# ,"1.8":"有小孩","0.18":"沒有小孩","1.9":"通勤","0.19":"無通勤","0.20":"點擊廣告"})
df2

from sklearn import tree

features = list(df2.columns[:30])
X = df2[features]
y = df2["有無點擊廣告"]
...
transact =df.groupby(["DATE","ID","MCC_CAT"])
...
MCC_CAT_ARRAY2=[]
c=0
for key,group in transact:
  c=c+1
  #print(key[0],key[1],key[2])
  if(flag==0):
    id=key[1]
    flag=1
    MCC_CAT_ARRAY.append(id)
    MCC_CAT_ARRAY.append(key[2])
    MCC_CAT_ARRAY2.append(key[2])
    i=i+1
  else:
    if(key[1]==id):
      MCC_CAT_ARRAY.append(key[2])
      ...
      flag=0
      datatable.append(MCC_CAT_ARRAY)#ID、消費種類one hot
      datatable2.append(MCC_CAT_ARRAY2)#ID、消費種類one hot
      MCC_CAT_ARRAY=[]
      MCC_CAT_ARRAY2=[]
print(datatable)
# with open('/content/drive/My Drive/Colab Notebooks/bigdata/hw3_task2.csv', 'w', newline='') as csvfile:
#   writer = csv.writer(csvfile)
#   # 寫入二維表格
#   writer.writerows(datatable)
# print("資料寫入完成")
...
onehot=te.fit(datatable2).transform(datatable2)
print(onehot)
print(te.columns_)

import pandas as pd
df2=pd.DataFrame(onehot,columns=te.columns_)
df2

from mlxtend.frequent_patterns import apriori
itemset=apriori(df2,min_support=0.004,use_colnames=True)
itemset['length']=itemset['itemsets'].apply(lambda x: len(x))
itemset.sort_values('support', ascending = False).head(40)
...
#plt.legend(loc = "center left",prop = font)
# 設定圖片標題，以及指定字型設定，x代表與圖案最左側的距離，y代表與圖片的距離
plt.title("付款別比例統計", x=0.5, y=1.03,FontProperties=font)
# 畫出圓餅圖
plt.show()
plt.close()
...
y4=0
for key,group in data:
  if(key[0]=="男"and key[15]==1):
    if(key[16]==1):
      y=y+1
    else:
      n=n+1
  if(key[13]=="電器用品"and key[15]==1):
    if(key[16]==1):
      y1=y1+1
    else:
      n1=n1+1
print("==========電器用品===============")
print("有點擊廣告:",y1,"沒有點擊廣告:",n1)
print("==========男性===================")
print("有點擊廣告:",y,"沒有點擊廣告:",n)
...
for key,group in data:
  ...
  #print(c)
#性別============================================
  if(key[0]=="女" and key[15]==1):
    if(key[16]==1):
      y1=y1+1
    else:
      n1=n1+1
  elif(key[0]=="男" and key[15]==1):
    if(key[16]==1):
      y31=y31+1
    else:
      n31=n31+1
...
#年齡============================================  
  if(key[1]<=39 and key[1]>=30and key[15]==1):
    if(key[16]==1):
      y2=y2+1
    else:
      n2=n2+1
  elif(key[1]<=49 and key[1]>=40and key[15]==1):
    if(key[16]==1):
      y3=y3+1
    else:
      n3=n3+1
  elif(key[1]<=59 and key[1]>=50and key[15]==1):
    if(key[16]==1):
      y4=y4+1
    else:
      n4=n4+1
  else:
    if(key[16]==1and key[15]==1):
      y5=y5+1
    elif(key[16]==0 and key[15]==1):
      n5=n5+1
...
#年收入============================================
  ...
  if(int(temp)>300000 and int(temp)<=500000and key[15]==1):
    if(key[16]==1):
      y9=y9+1
    else:
      n9=n9+1
  elif(int(temp)>500000 and int(temp)<=700000and key[15]==1):
    if(key[16]==1):
      y10=y10+1
    else:
      n10=n10+1
  else:
    if(key[16]==1 and key[15]==1):
      y12=y12+1
    elif(key[16]==0 and key[15]==1):
      n12=n12+1
...
#消費種類============================================ 
  ...
  if(key[13]=="服飾用品"and key[15]==1):
    if(key[16]==1):
      y13=y13+1
    else:
      n13=n13+1
  elif(key[13]=="美妝保健"and key[15]==1):
    if(key[16]==1):
      y14=y14+1
    else:
      n14=n14+1
  elif(key[13]=="電器用品"and key[15]==1):
    if(key[16]==1):
      y15=y15+1
    else:
      n15=n15+1
  elif(key[13]=="生活百貨"and key[15]==1):
    if(key[16]==1):
      y16=y16+1
    else:
      n16=n16+1
  elif(key[13]=="教育娛樂"and key[15]==1):
    if(key[16]==1):
      y17=y17+1
    else:
      n17=n17+1
  elif(key[13]=="食品飲料"and key[15]==1):
    if(key[16]==1):
      y18=y18+1
    else:
      n18=n18+1
  elif(key[13]=="其他"and key[15]==1):
    if(key[16]==1):
      y19=y19+1
    else:
      n19=n19+1
...
#付款別==============================================
  if(key[11]=="貨到付款"and key[15]==1):
    if(key[16]==1):
      y20=y20+1
    else:
      n20=n20+1
  elif(key[11]=="信用卡"and key[15]==1):
    if(key[16]==1):
      y21=y21+1
    else:
      n21=n21+1
  elif(key[11]=="ATM轉帳"and key[15]==1):
    if(key[16]==1):
      y22=y22+1
    else:
      n22=n22+1
...
#結婚與否============================================
  if(key[2]==0and key[15]==1):
    if(key[16]==1):
      y23=y23+1
    else:
      n23=n23+1
  elif(key[2]==1and key[15]==1):
    if(key[16]==1):
      y24=y24+1
    else:
      n24=n24+1
#交通工具與否============================================
  if(key[6]==0and key[15]==1):
    if(key[16]==1):
      y25=y25+1
    else:
      n25=n25+1
  elif(key[6]==1and key[15]==1):
    if(key[16]==1):
      y26=y26+1
    else:
      n26=n26+1
#小孩與否============================================
  if(key[8]==0and key[15]==1):
    if(key[16]==1):
      y27=y27+1
    else:
      n27=n27+1
  elif(key[8]!=0and key[15]==1):
    if(key[16]==1):
      y28=y28+1
    else:
      n28=n28+1
#通勤與否============================================
  if(key[7]==0and key[15]==1):
    if(key[16]==1):
      y29=y29+1
    else:
      n29=n29+1
  elif(key[7]==1and key[15]==1):
    if(key[16]==1):
      y30=y30+1
    else:
      n30=n30+1    
...
print("有點擊廣告:",y1,"沒有點擊廣告:",n1)
print("==========女===================")
print("有點擊廣告:",y31,"沒有點擊廣告:",n31)
print("==========30~39歲===============")
print("有點擊廣告:",y2,"沒有點擊廣告:",n2)
print("==========40~49歲===================")
print("有點擊廣告:",y3,"沒有點擊廣告:",n3)
print("==========50~59歲===============")
print("有點擊廣告:",y4,"沒有點擊廣告:",n4)
print("==========其他年齡===================")
print("有點擊廣告:",y5,"沒有點擊廣告:",n5)
print("==========北部===============")
print("有點擊廣告:",y6,"沒有點擊廣告:",n6)
print("==========中部===================")
print("有點擊廣告:",y7,"沒有點擊廣告:",n7)
print("==========其他地區===============")
print("有點擊廣告:",y8,"沒有點擊廣告:",n8)
print("==========年收入30w===================")
print("有點擊廣告:",y9,"沒有點擊廣告:",n9)
print("==========年收入50w===============")
print("有點擊廣告:",y10,"沒有點擊廣告:",n10)
print("==========年收入其他===================")
print("有點擊廣告:",y12,"沒有點擊廣告:",n12)
print("==========服飾用品===============")
print("有點擊廣告:",y13,"沒有點擊廣告:",n13)
print("==========美妝保健===================")
print("有點擊廣告:",y14,"沒有點擊廣告:",n14)
print("==========電器用品===============")
print("有點擊廣告:",y15,"沒有點擊廣告:",n15)
print("==========生活百貨===================")
print("有點擊廣告:",y16,"沒有點擊廣告:",n16)
print("==========教育娛樂===================")
print("有點擊廣告:",y17,"沒有點擊廣告:",n17)
print("==========食品飲料===============")
print("有點擊廣告:",y18,"沒有點擊廣告:",n18)
print("==========其他===================")
print("有點擊廣告:",y19,"沒有點擊廣告:",n19)
print("==========貨到付款===============")
print("有點擊廣告:",y20,"沒有點擊廣告:",n20)
print("==========信用卡===================")
print("有點擊廣告:",y21,"沒有點擊廣告:",n21)
print("==========ATM轉帳===============")
print("有點擊廣告:",y22,"沒有點擊廣告:",n22)
print("==========已婚===================")
print("有點擊廣告:",y23,"沒有點擊廣告:",n23)
print("==========未婚===============")
print("有點擊廣告:",y24,"沒有點擊廣告:",n24)
print("==========有交通工具===================")
print("有點擊廣告:",y25,"沒有點擊廣告:",n25)
print("==========無交通工具===============")
print("有點擊廣告:",y26,"沒有點擊廣告:",n26)
print("==========有小孩===================")
print("有點擊廣告:",y27,"沒有點擊廣告:",n27)
print("==========沒有小孩===============")
print("有點擊廣告:",y28,"沒有點擊廣告:",n28)
print("==========通勤===================")
print("有點擊廣告:",y29,"沒有點擊廣告:",n29)
print("==========無通勤===================")
print("有點擊廣告:",y30,"沒有點擊廣告:",n30)